$(document).ready(function(){
                ...
                }).on("loaded.rs.jquery.bootgrid", function()
                {
                    ...
                    {
                        window.location.href = "/admin/principi-attivi/" + $(this).data("row-id") + "/modifica";
                    ...
                    }).end().find(".command-delete").on("click", function(e)
                    {
                            ...
                            if(prodotti == 0){
                                swal({   
                                    title: "Sei sicuro di voler cancellare questo principio attivo?",   
                                    ...
                                    swal("Cancellato!", "Principio attivo è stato cancellato con successo.", "success"); 
                                    deleteResource(id, 'principi-attivi');
                                    $("#data-table-command").bootgrid('remove', rows);
                                });
                            }else{
                                swal({   
                                    title: "Impossibile cancellare Categoria Terapeutica.",   
                                    text: "Non è possibile cancellare questo principi attivo perchè è associato a " + prodotti + " prodotti.",   
                                    timer: 5000,   
